...
class VodTrailerPageMobileState extends State<VodTrailer> {
          ...
          children: <Widget>[
            if (!_castConnected)
              PlayerButtons.seekBackward(
                  onPressed: _controller.seekBackward, color: Colors.white),
            const SizedBox(width: 16),
            createPlayPauseButton(),
            const SizedBox(width: 16),
            if (!_castConnected)
              PlayerButtons.seekForward(
                  onPressed: _controller.seekForward, color: Colors.white)
          ]);
    });
  }
  ...
  Widget _timeLine() {
    return ChromeCastInfo().castConnected
        ? const SizedBox()
        : LitePlayerTimeline(_controller);
  }
  ...
  void _chromeCastCallback(bool connected) {
    if (connected) {
      _controller.dispose();
      ChromeCastInfo()
          .initVideo(widget.link, AppLocalizations.toUtf8(widget.title));
    } else {
