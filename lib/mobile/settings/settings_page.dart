...
import 'package:jarambahtv/mobile/settings/age_picker.dart';
import 'package:jarambahtv/mobile/settings/epg_dialog.dart';
import 'package:jarambahtv/pages/debug_page.dart';
import 'package:jarambahtv/service_locator.dart';
import 'package:jarambahtv/shared_prefs.dart';
import 'package:jarambahtv/theme/color_picker.dart';
import 'package:jarambahtv/theme/theme.dart';
import 'package:jarambahtv/theme/theme_picker.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
            ...
            title:
                Text(_translate(TR_SETTINGS), style: TextStyle(color: color))),
  ...
  Widget build(BuildContext context) {
    return ListTile(
        leading: SettingsIcon(
            _saveLastViewed ? Icons.bookmark : Icons.bookmark_border),
...
class _ControlsState extends State<_Controls> {
  ...
  void initState() {
    ...
    final settings = locator<LocalStorageService>();
    _value =
        widget.type == 0 ? settings.soundChange() : settings.brightnessChange();
  }
  ...
  Widget _tile(bool sound) {
    return ListTile(
        ...
        title:
            Text(_translate(sound ? TR_SOUND_CONTROL : TR_BRIGHTNESS_CONTROL)),
  ...
  void _onTap() {
    setState(() {
      ...
      _value = !_value;
      _isSound
          ? settings.setSoundChange(_value)
          : settings.setBrightnessChange(_value);
    });
  }
