  ...
  Widget _typeTile(PickStreamFrom value) {
    return ListTile(
        title: Text(AppLocalizations.of(context).translate(
            value == PickStreamFrom.SINGLE_STREAM
                ? TR_SINGLE_STREAM
                : TR_PLAYLIST)),
        leading: Radio(
            autofocus: true,
            groupValue: _source,
            value: value,
            onChanged: (PickStreamFrom value) =>
                setState(() => _source = value)));
  }

  Widget _button(String text, void Function() onPressed, double opacity) {
    final buttonTextColor =
        Theming.of(context).onBrightness().withOpacity(opacity);
    return FlatButton(
        child: Text(text, style: TextStyle(color: buttonTextColor)),
        onPressed: onPressed);
  }
