...
import 'package:jarambahtv/base/tabbar.dart';
...
import 'package:jarambahtv/tv/add_streams/tv_add_stream_dialog.dart';
import 'package:jarambahtv/tv/add_streams/tv_stream_quantity.dart';
import 'package:jarambahtv/tv/exit_dialog.dart';
import 'package:jarambahtv/tv/search_page.dart';
import 'package:jarambahtv/tv/settings/tv_settings_page.dart';
import 'package:jarambahtv/tv/streams/tv_live_tab.dart';
import 'package:jarambahtv/tv/vods/tv_vod_tab.dart';
import 'package:flutter/material.dart';
import 'package:flutter_common/widgets.dart';

class HomeTV extends HomePage {
  const HomeTV(List<LiveStream> channels, List<VodStream> vods)
      : super(channels, vods);
...
class _HomeTVState extends VideoAppState with TickerProviderStateMixin {
  ...
  void _initTabController() {
    ...
    _tabController = TabController(
        vsync: this,
        length: videoTypesList.length,
        initialIndex: _initIndex < 0 ? 0 : _initIndex);
  ...
  void _onSearch() async {
    ...
    final stream = await Navigator.push(context,
        MaterialPageRoute(builder: (context) => SearchPage(_currentStreams)));
  ...
  void _toSettings() async {
    final tvTabsEvents = locator<TvTabsEvents>();
    tvTabsEvents.publish(OpenedTvSettings(true));
    final double padding = await Navigator.push(context,
        MaterialPageRoute(builder: (context) => const SettingPageTV()));
    tvTabsEvents.publish(OpenedTvSettings(false));
  ...
  void _onAdd() async {
    final PickStreamFrom _source = await showDialog(
        context: context,
        builder: (BuildContext context) => const StreamTypePickerTV());
    if (_source != null) {
