...
import 'package:jarambahtv/base/streams/live_timeline.dart';
import 'package:jarambahtv/base/streams/program_bloc.dart';
import 'package:jarambahtv/base/streams/program_time.dart';
import 'package:jarambahtv/base/tv/constants.dart';
import 'package:jarambahtv/base/tv/snackbar.dart';
import 'package:jarambahtv/bloc/live_bloc.dart';
import 'package:jarambahtv/channels/live_stream.dart';
import 'package:jarambahtv/events/ascending.dart';
...
import 'package:jarambahtv/notification.dart';
import 'package:jarambahtv/player/controller.dart';
...
import 'package:jarambahtv/tv/streams/common_widgets.dart';
import 'package:jarambahtv/tv/streams/tv_live_channels.dart';
import 'package:jarambahtv/tv/streams/tv_live_edit_channel.dart';
...
class _ChannelsTabHomeTVState extends State<ChannelsTabHomeTV> {
  ...
  void initState() {
    ...
    tvTabsEvent
        .subscribe<TvGuideSwitch>()
        .listen((event) => _onGuideEvent(event.onScroll));
  ...
  Widget page() {
    ...
    return Row(
        ...
        children: <Widget>[
          Visibility(
              visible: notFullScreen, child: channelsList(availableSpace)),
          Visibility(
              visible: notFullScreen, child: const VerticalDivider(width: 0.0)),
          Expanded(
              child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
            playerArea(availableSpace),
            channelInfo(availableSpace, scale)
          ])),
  ...
  Widget playerArea(Size availableSpace) {
    return TvPlayerWrap(
        LitePlayer(controller: _controller), !notFullScreen, _onPlayer);
  }
    ...
    return Visibility(
        ...
        child: Padding(
            ...
            child: Column(
                ...
                children: <Widget>[
                  _TimeLine(programsBloc,
                      Size(availableSpace.width * 0.45, 24 * scale)),
                  Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: <Widget>[
                        Expanded(
                            child: Text(
                                AppLocalizations.toUtf8(_playing.displayName()),
                                style: TextStyle(fontSize: 18 * scale))),
                        FavoriteStarButton(_playing.favorite(),
                            onFavoriteChanged: (bool value) {
                          widget.bloc
                              .handleFavorite(!_playing.favorite(), _playing);
                        }, unselectedColor: Theming.of(context).onBrightness()),
                        IconButton(
                            icon: const Icon(Icons.edit), onPressed: _edit),
                        IconButton(
  ...
  Widget programs(Size availableSpace) {
    return Programs(
        Size(availableSpace.width * 0.3, availableSpace.height - 72),
        programsBloc);
  }
  ...
  void _showSnackBar(bool show) {
    ...
    if (show) {
      ...
      final snack = PlayerSnackbarTV(
          context, _playing.displayName(), _controller.isPlaying());
  ...
  bool _onChannel(FocusNode node, RawKeyEvent event, int index) {
    return onKey(event, (keyCode) {
      switch (keyCode) {
        ...
        case KEY_DOWN:
          if (_updateGuideOnScroll &&
              currentChannelEPG != _currentChannels.length - 1) {
            currentChannelEPG++;
            _setPrograms(currentChannelEPG);
          }
  ...
  void setFullscreenOff(bool visibility) {
    ...
    TvChannelNotification(
            title: NotificationTypeTV.FULLSCREEN, visibility: notFullScreen)
        .dispatch(context);
    final settings = locator<LocalStorageService>();
    if (notFullScreen) {
