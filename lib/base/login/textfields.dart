...
class _LoginTextFieldState extends State<LoginTextField> {
  ...
  void initState() {
    super.initState();
    _controller =
        widget.controller ?? TextEditingController(text: widget.init ?? '');
  ...
  Widget build(BuildContext context) {
    ...
    return Focus(
        ...
        child: Padding(
            padding: const EdgeInsets.all(TEXTFIELD_PADDING), child: child));
  }
  ...
  OutlineInputBorder _border() {
    if (widget.mainFocus?.hasPrimaryFocus ?? false) {
      return OutlineInputBorder(
          borderSide:
              BorderSide(color: Theme.of(context).accentColor, width: 4));
    }
    return OutlineInputBorder(
        borderSide: BorderSide(color: Theming.of(context).onBrightness()));
  }
  ...
  OutlineInputBorder _errorBorder() {
    ...
    if (widget.mainFocus?.hasPrimaryFocus ?? false) {
      border =
          border.copyWith(borderSide: border.borderSide.copyWith(width: 4));
    }
    return border;
  }
