...
class M3UParser {
  ...
  LiveStream _createLiveStream(Map<String, dynamic> m3u) {
    ...
    final _channelInfo = ChannelInfo(m3u[ID_FIELD], m3u[GROUP_FIELD], 0, false,
        0, 0, false, _epg, true, true, null, 0, <MetaUrl>[], m3u[GROUP_TAG]);

    return LiveStream(_channelInfo, _epgLink);
  }

  VodStream _createVodStream(Map<String, dynamic> m3u) {
    final _movieInfo = MovieInfo([m3u[PRIMARY_URL_FIELD]], '', '',
        m3u[NAME_FIELD], m3u[ICON_FIELD], '', 0.0, 0, '', 0, MovieType.VODS);
    final vodInfo = VodInfo(m3u[ID_FIELD], m3u[GROUP_FIELD], 21, false, 0, 0,
        false, _movieInfo, true, true, null, 0, <MetaUrl>[]);
