...
class LiveTime extends StatefulWidget {
  ...
  const LiveTime.current({@required this.programmeInfo, this.color})
      : isLive = true;

  const LiveTime.end({@required this.programmeInfo, this.color})
      : isLive = false;
...
class LiveTimeState<T extends LiveTime> extends State<T> {
  ...
  Widget build(BuildContext context) {
    final color =
        widget.color ?? Theming.onCustomColor(Theme.of(context).primaryColor);
  ...
  void initTimeline(ProgrammeInfo programmeInfo) {
    ...
    if (widget.isLive) {
      _timer = Timer.periodic(const Duration(seconds: REFRESH_TIMELINE_SEC),
          (Timer t) => _update(programmeInfo));
    }
  }
  ...
  String _parse(int time) =>
      TimeParser.hms(time - DateTime.now().timeZoneOffset.inMilliseconds);
}
