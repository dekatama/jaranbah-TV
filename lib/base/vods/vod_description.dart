...
import 'package:jarambahtv/mobile/vods/vod_player_page.dart';
import 'package:jarambahtv/mobile/vods/vod_trailer_page.dart';
...
class SideInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> info = [
      ...
      _sideDescription(translate(context, TR_DURATION),
          data: _getDuration(duration)),
  ...
  Widget _sideDescription(String title, {String data}) {
    return SideInfoItem(
        title: AppLocalizations.toUtf8(title ?? ''), data: data);
  }
}
...
class VodTrailerButton extends StatelessWidget {
  ...
  void _onTrailer(BuildContext context) {
    Navigator.of(context).push(MaterialPageRoute(builder: (context) {
      return VodTrailer(
          translate(context, TR_TRAILER) + ": " + channel.displayName(),
          channel.trailerUrl(),
          channel.previewIcon());
    }));
  }
...
class DescriptionText extends StatelessWidget {
  ...
  const DescriptionText(this.text,
      {this.scrollController, this.textColor, this.textSize});
        ...
        : SingleChildScrollView(
            ...
            child: Padding(
                ...
                child: Text(AppLocalizations.toUtf8(text),
                    style: TextStyle(
                        fontSize: textSize ?? 16, color: textColor))));
  }
}
