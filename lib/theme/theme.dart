...
class _ThemingState extends State<Theming> {
  ...
  ThemeData darkTheme =
      ThemeData.dark().copyWith(primaryColor: Colors.grey[900]);
  ...
  Color onAccent({Color dark, Color light}) {
    if (ThemeData.estimateBrightnessForColor(theme.accentColor) ==
        Brightness.dark) {
      return dark ?? Colors.white;
    } else {
      return light ?? Colors.black;
    ...
    return copyWith(
        ...
        elevatedButtonTheme: ElevatedButtonThemeData(
            style: ElevatedButton.styleFrom(
                primary: color, onPrimary: Theming.onCustomColor(color))),
